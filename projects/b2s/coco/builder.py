...
from .sample import LoTSS_B2S_Sample, LoTSS_Search_Sample, LoTSS_Negative_GRG_Sample
from .category import LoTSS_B2S_CocoCategory
from .clean import COCODatasetCleaner
from .evaluator import GTEvaluator

from b2s.annotations import annotate_and_augment, augment_and_get_proposals, annotate, GRGFinder
...
B2S_CATEGORY = LoTSS_B2S_CocoCategory(id=1, name="radio_source")
...
class B2SDatasetBuilder(CocoDatasetBuilderBase):
    ...
    def _add_categories(self, coco: dict) -> dict:
        coco['categories'].append(B2S_CATEGORY.to_dict())
        return coco
    ...
    def _generate_positive_samples(self, cutout, cutout_index, coco, coco_lock) -> int:
        ...
        try:
            ...
            for angle_index, angle in enumerate(self.rotation_angles):               
                curr_gt_component_membership = (
                    gt_component_membership[angle_index]
                    if isinstance(gt_component_membership, list)
                    else gt_component_membership
                )
                curr_gt_proposal_validity = (
                    gt_proposal_validity[angle_index]
                    if isinstance(gt_proposal_validity, list)
                    else gt_proposal_validity
                )
                curr_features = (
                    features[angle_index]
                    if isinstance(features, list)
                    else features
                )
                curr_within_proposal_mask = (
                    within_proposal_mask[angle_index]
                    if isinstance(within_proposal_mask, list)
                    else within_proposal_mask
                )
                ...
                sample = LoTSS_B2S_Sample(
                    ...
                    full_annotation=(curr_gt_component_membership, curr_gt_proposal_validity),
                    ...
                    within_proposal_mask=curr_within_proposal_mask,
    ...
    def _get_positions(
        ...
        segment_positions = {}
        for source_name, (x_pos, y_pos) in zip(candidates["source_names"], candidates["xy_list"]):
    ...
    def _get_positions_negative_grg(
        ...
        positions = {}
        for component_name, (x_pos, y_pos) in zip(candidates["component_names"], candidates["xy_list"]):
...
class GRGSearchDatasetBuilder(B2SDatasetBuilder):
    def __init__(
            self,
            cutouts: list,
            component_catalogue: pd.DataFrame,
