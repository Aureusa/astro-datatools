...
def annotate_and_augment(
    ...
    physical_quantities_arr = np.array(candidates_values[3:]) # holds things like total flux peak flux, maj, min, etc. - shape (num_physical_quantities, num_components)
    
    # TODO: Possibly refactor this at some point ---------------------------------------------
    # Check for the order and the presence of the phyical quantities in the candidates dict,
    # as we rely on this order for the feature derivation, and if they are not present we will
    # get an error later on when we try to access them by index
    enforce_phyiscal_quantities_order = ["total_flux", "peak_flux", "maj", "min"]
    for i, quantity in enumerate(enforce_phyiscal_quantities_order):
        if quantity not in candidates_keys:
            raise ValueError(f"Missing required physical quantity '{quantity}' in candidates dict for feature derivation.")
        if candidates_keys.index(quantity) != i + 3: # The physical quantities start from index 3 in the candidates values
    ...
    physics_features = PhysicsAwareFeatures(
        ...
        physical_quantities=physical_quantities_arr,
        indeces_to_keep=original_indices_to_keep_after_segmentation_list,
        max_islands=max_precomputed_islands,
    )
